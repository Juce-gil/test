        ...
        <el-row :gutter="30">
          ...
          <el-col :span="12">
            ...
            <el-form-item label="商品类别" prop="categoryId">
              <el-select v-model="product.categoryId" placeholder="请选择商品类别">
                <el-option
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        oldLevel: [{ required: true, message: '请选择新旧程度', trigger: 'change' }],
  ...
  methods: {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          if (body && body.code === 200 && body.data != null) {
            const url = typeof body.data === 'string' ? body.data : (body.data.url || body.data.path || body.data.fileUrl || '')
            if (url) options.onSuccess(url)
            else options.onError(new Error('返回的图片地址无效'))
          } else {
            options.onError(new Error(body && (body.msg || body.message) || '上传失败'))
          }
        })
        .catch(err => {
          options.onError(err)
    ...
    handleUploadSuccess(url) {
      const raw = typeof url === 'string' ? url : (url && (url.url || url.path || url.fileUrl))
